...
- TODO: Add browser e2e asserting the scene **background** AND **tokens** render (Scene `system.background` → sprite; `doc_type:"token"` → tweened sprite). The M8c-1 `SceneReconciler` + the M8d-1 `TokenView`/`TokenAnimator` are unit-tested against a mock backend, but the live render assertions need authored documents, which need the scene/place UI that lands in **M8d-2**. Add both to the Playwright stage smoke once the M8d-2 place tool can author a scene + token. (Deferred per the user-approved decomposition, not a descope.)
