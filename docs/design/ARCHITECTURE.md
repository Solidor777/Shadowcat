...
Source resides under `src/`: `src/server/` (Rust workspace), `src/client/{core,ui}/` (headless core + Svelte default UI), `src/modules/` (first-party default modules), `src/types/` (generated TS types). The Rust server binary builds to `target/` (Cargo convention); the client bundle builds to `dist/`. Binary assets shipped with the app (e.g., the icon) live in `assets/` at the repo root, outside `src/`.
