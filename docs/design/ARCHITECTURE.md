...
**Target client UI packaging (decomposition planned — `PLAN.md` M8.5).** The browser client resolves into three replaceable layers: a thin **app shell** (bootstrap, routing, session, the framework-neutral surface host); a **swappable entry package** (login + world management, pre-world) a self-hoster can replace to integrate external auth/identity; and **per-element in-game UI packages** (each region / panel / tool / HUD its own module under `src/modules/*`) so any in-game element is independently moddable or replaceable. Today `@shadowcat/ui` still bundles all three. The enabling seams already exist (the M6b/M7 module + `provides`/`requires` contract system, `ContributionRegistry`, the surface host, the engine-owned render-layer API), and the **discipline is in force now**: in-game elements communicate only through these seams — never importing one another directly — so the eventual extraction is mechanical, not a rewrite. New in-game UI (from M8d) ships as its own `src/modules/*` package.
...
6. **Module and system logic runs on the client, GM-authoritative.** The server is relay + persistence + structural validation; it runs no third-party code in v1. The server's authority over module-computed state and the `system` body is **structural only** — size caps, field-path validity, `deny_unknown_fields`, and permissions; it performs **no semantic/mechanical validation** of system-defined content. GM-originated intents carrying module-computed state are accepted as authoritative on that basis. This is the cooperative-play trust model (install-time trust: only a GM activates modules) and is what keeps the server free of third-party code and the binary single. **Exception — engine-owned geometric systems (M9):** the server runs two server-authoritative *geometric* checks, reading wall/token geometry from the per-world scene ECS — **movement-collision** (a non-GM token move that crosses a `blocksMove` wall is rejected before the write, M9a) and **per-player vision** (visibility raycasting, M9b). These are engine-owned (not module or third-party code), are the only semantic reads of `system` the server makes, and exist because a stray/modified client must never produce an authoritative position behind a wall or a vision leak. They interpret only geometry, never module-defined mechanics.
