...
6. **Three-band document shape: envelope name, typed `engine`, opaque `system` (M13-0).** Every document carries an envelope-level `name: Option<String>` (a universal display name, redacts to `null` under a per-property override — never stripped, so the envelope shape stays stable for every recipient) alongside two content bodies with different authority models:
   - **`engine`** — present iff `doc_type` is one of the 26 engine-defined types (tokens, actors, scenes, walls, regions, lights, drawings, templates, messages, world/vision/lighting/chat/dice config-docs, the faction/condition/channel registries, asset folders (`asset_folder`), rollable tables (`table`), rich-text notes (`note`), the world's system-declared defaults (`system-defaults`), and the combat family: `combat`, `combatant`, `resource-registry`, `effect`, `combat-history`) — see `data::engine::is_engine_doc_type` for the exhaustive list. Typed, ts-rs-generated Rust structs (`src/server/src/data/engine/`) with **strict ingress validation**: `validate_engine`/`validate_engine_tree` reject an unknown field, a wrong-typed field, an engine body on a non-engine `doc_type`, or a missing `engine` body on an engine `doc_type` — checked on every Create/Update post-image, including embedded children, at one recursive chokepoint. This is the band engine-owned geometry (walls, regions, vision, tokens, movement) now lives in, replacing the pre-M13-0 practice of that geometry sharing the opaque `system` root.
   - **`system`** — opaque, system-defined JSONB body. The server's authority over what a value *means* is nil: it performs no semantic or mechanical validation of system content and never interprets it. Its authority over *structure* is total (size caps, JSON validity, `deny_unknown_fields`, declared shape schemas, permissions). This is the band game-system modules own.
   - **The server evaluates the engine's own grammars over `system` data a system names, and interprets declarative data it owns the grammar of** — the formula language (`@shadowcat/formula` and its server twin `crate::formula`) over numeric `system` leaves a formula references, dice notation, the M13f schema registry. None of that is interpreting system semantics: the server reads `system.stats.speed.final` because a formula told it to, and knows nothing about what speed is. It never executes third-party code.
   - **By default, computation runs on the server; the client requests and the server answers.** System semantics are defined by system modules, but the system is assumed to run against the server: a derived value a system wants the engine to act on (a resource ceiling, a duration, a lifecycle flag) is a persisted `system` leaf the server evaluates from, never a number a browser computed and wrote. Client-side computation is the exception and needs a reason — presentation, input capture, optimistic prediction. The cooperative-play trust model still governs *what a GM may author*; it no longer makes a GM's client the authority for anything the engine derives. The `engine` band is server-validated engine territory, not module-owned; movement-collision and per-player vision (M9) read `engine` geometry directly, since that geometry lives in the typed, server-validated band by construction.
...
| ~~VFX~~ (built: server-derived grid sheets, a `vfx` core layer, per-token emitters + room-wide one-shots, the `/fx` command), ~~multi-level maps/portals~~ (built: `SceneLevel`/`ElevationBand`, `TriggerEffect::Teleport`, per-level vision/movement/render scoping), post-processing, advanced vision modes | render-layer abstraction; ECS components | Phase 2–3, after the gameplay loop is proven. |
...
- **Re-parenting is a first-class, GM-only document operation.** A document's envelope `parent_id`
  is immutable via field-path `Update` (`required_cap_for_path` maps no capability to envelope
  fields); the ONE write path for it is `Operation::Move { doc_id, parent_id, old_parent_id }`,
  riding the ordinary intent pipeline. Its rules, all enforced at the `apply_intent` chokepoint:
  authorization is GM-only AND uncapped (`WorldRole::Gm` whose resolved `Access.all`
  short-circuit holds — a `gm_role`-capped GM floor-resolves and is refused); validity is
  **Create-validity** — a Move is legal exactly where creating the document with the target
  parent would be, routed through the same shared placement checks the Create arm uses
  (`check_parent_placement`), never a parallel copy; a batch-aware ancestor walk
  (`check_move_acyclic`) refuses any move of a document beneath itself; `old_parent_id` is the
  OCC pre-image and `invert` swaps the pair; a no-op move (target equals current) commits
  nothing and runs no hooks. Per-type post-move hooks live at the same chokepoint: a moved
  `asset_folder` recomputes its asset subtree's derived tags, a moved `token`/`region`
  re-derives scene membership in the ECS mirror (membership is derived from the doc's own
  `parent_id`, so the source scene forgets it and the destination gains it). On egress a Move
  carries only envelope placement — no content bands — so redaction reduces to the
  whole-document READ conjunction (`filter_command`'s Move arm: delivered iff READ at commit AND
  READ now); no READ transition can arise from a Move alone, since access resolution never
  consults `parent_id`. Embedded children are out of scope: Move rewrites `parent_id` only and
  never restructures `embedded`.
- **Stable asset identity.** Assets are referenced by stable UUID from first upload, so moving, renaming, replacing or reconverting an asset never breaks links. On disk the UUID is the canonical served file (`<assets_dir>/<world>/<uuid>`); the retained original and the thumb/preview derivatives are siblings (`<uuid>.orig`, `<uuid>.thumb.webp`, `<uuid>.preview.webp`) that every file-op moves as one set, and `version` — bumped only by a byte swap — keys the ETag of the canonical and its derivatives alike. Chat images extend the same identity model with a `ChatImage` provenance: an author-placed `[[asset:<uuid>|alt]]` span references an existing asset directly, while a Markdown/HTML image URL in a message body is fetched server-side (the SSRF-guarded link-preview pipeline) and asset-ified in the background, so a chat `image` segment always resolves through the identical stable-UUID serving path — never a raw, client-fetched external URL.
