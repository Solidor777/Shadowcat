...
| `src/modules/stage/src/Stage.svelte` | M22 (`{#key antialias}`, `data-*` attrs, `performance` opt), M24 (`vfxAssets`, `playVfx`), M25 (level scoping, `data-level`), M26 (`STAGE_OVERLAY_CONTRACT` surface) | Each milestone adds its own block; no milestone reorders existing markup. |
