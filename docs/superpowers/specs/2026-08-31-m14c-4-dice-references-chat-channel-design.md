...
| # | Decision |
|---|---|
| R1 | **The server gains `formula::template`, a behavioural twin of the TS `template` module's rewrite half.** Same recognizer chain, same keyword reservation, same `1d` count synthesis, same labeled substitution (`3[str]`, `-2[str]` for negatives), same integer-only and i32-magnitude rules, same error kinds and `detail` wording, same UTF-16 position counting. Pinned by the shared conformance corpus (R6), never by convention. `checkNotationKey` is NOT ported — it is an authoring aid with no server consumer; the server answers "does this reference run" by running it. **The chain gained one recognizer in flight:** `claimNotationFunction` — a `NOTATION_FUNCTIONS` run (the dice grammar's six `fn_call` names) immediately followed by `(` is notation, never a reference. Server-side resolution runs EVERY roll through the scan, so without this rule `floor(101d6/2)` would break; the client preview flow never executed the scan against real rolls and the hole was invisible. Both twins gain it in the same commit, corpus-pinned. |
| R2 | **Resolution is a pre-parse substitution at the transport boundary, inside the one execution path.** `chat::rolls::execute_roll`/`execute_roll_with_seed` gain a host parameter (`validate_formula` deliberately takes none — its placeholder resolver is internal, see §4); the template is resolved to labeled-literal notation FIRST and the existing `notation::parse` + caps run on the result. The dice grammar itself is untouched — a substituted reference is exactly the labeled-constant shape `1d20 + 3[str]` the parser already accepts, and `RollOutcome.labeled_consts` keeps the reference as display provenance. The stored `RollEmbed.formula` keeps the author's template text verbatim; `spec`/`raw` describe the substituted roll, so a later GM recalculation re-derives from the stored naturals and never re-resolves (a stat change after the roll must not rewrite history). |
...
- **The recognizer chain** — `Claim { kind, text }`, `NotationClaimKind`
  ...
  not exported), the recognizers in the same order with the same observability rules
  (`claimLabelSpan`'s rejecting branch, `claimNotationKeyword` ahead of `claimIdentifierSpan` being
  the one observable adjacency — `claimNotationFunction` sits between them and is unobservable
  there, the keyword and function sets being disjoint), `claim_at` total via the literal
  fallback. `claimNotationFunction` reserves a `NOTATION_FUNCTIONS` run (the six `fn_call`
  names, declared in the module beside `NOTATION_KEYWORDS` and kept OUT of that list — it guards
  the modifier vocabulary) only when immediately followed by `(`, matching the dice parser's own
  `fn_call` rule; the parity gate extends to this second vocabulary (§parity in R7's gate — the
  extractor reads the dice parser's `fn_call` arms and both template sides' `NOTATION_FUNCTIONS`).
...
- `embedded_actor_copy` (currently private in `combat::eval`, shared by `formula_host` and
  ...
  binding.
- `chat` gains `host_for_actor_owner(repo, &ActorOwnerRef) ->
  Result<Option<Document>, DataError>` — as built (no `world_id` parameter; it re-reads the
  documents rather than keeping the ingest gate's fetch, so every case costs one lookup, the
  `TokenInstance`-without-embedded-copy case two): `Actor` → the actor doc; `TokenInstance` → the token doc's
  ...
  gate in `handle_send_message` has already validated the ref before this is ever called — the
  function performs no authorization of its own (the contract is documented on it).
