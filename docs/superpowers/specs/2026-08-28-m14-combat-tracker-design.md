...
| D4 | *Amended (M14c-1):* the server evaluates formulas; see the [M14c-1 spec](2026-08-30-m14c-1-server-formula-engine-design.md). Original text: **Formulas are opaque to the server** (ARCHITECTURE invariant 6 kept). Resource definitions store formulas as strings; the CLIENT (active system's resolver via `@shadowcat/formula`) evaluates them and writes NUMBERS into the combatant. The server only ever reads and arithmetically updates numbers. |
...
| D13 | **Initiative is server-rolled** via a `CombatRoll` intent using the existing `chat::rolls` cap/entropy layer, written atomically into the combat doc and posted to chat with a `RollEmbed` (hidden ⇒ GM-only whisper). Client composes notation exactly as the M13d roll wire does. *Amended (M14c-4):* the wire carries the RAW template (`1d20 + init`); the server resolves each entry's references at execution against that combatant's formula host — "client composes" is superseded by server-side resolution, and `channel` is validated against the channel registry. |
