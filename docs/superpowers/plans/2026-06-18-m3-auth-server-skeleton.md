...
This plan implements security-sensitive auth (argon2 credential handling, signed DB-backed sessions, the open setup window + token, headless credential bootstrap, and non-enumerating login). That qualifies as high-risk. **Outcome of the checkpoint:** a buddy-check is **offered** to the user at the execution handoff (below); it is not auto-run. If accepted, run `buddy-checking` over the credential/session/setup-token surface — Tasks 2, 6, 7, 8 — after implementation, before merge. Record the user's decision here when made.
