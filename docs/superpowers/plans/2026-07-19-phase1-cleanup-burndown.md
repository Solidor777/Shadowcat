...
- **Standing directive (user, 2026-07-21): any new bug or TODO discovered during this burndown is fixed now, in-line, not deferred.** No deferral to `docs/TODO.md`/`docs/OPEN_BUGS.md` without the user's express permission — a discovery during a task's work is folded into that task (or dispatched as an immediate ad-hoc fix task inserted into the loop) before moving on. By the time the burndown finishes, every bug and TODO that CAN be closed without building a future/unbuilt feature must be closed. Items that are genuinely blocked on a not-yet-built feature remain the only legitimate deferral, and must still be logged (not silently dropped).
