...
### Task 4: Root-cause fix — boot() honors the URL's world route on reload

Rewritten 2026-07-31 (second amendment; the first — an escalating-watchdog hypothesis — was falsified by its own Step-1 gate and by dispatcher trace forensics). GROUND TRUTH from a captured Playwright network trace of the failing `panels.spec.ts:18` run: the reload's `GET /api/me/ui-state` returned `global.lastWorld` = a DIFFERENT test's world id, and the reloaded page entered THAT world. Mechanism: all 6 e2e workers share the `ops` account; every test's `enterWorld` persists `global.lastWorld`; `App.svelte`'s `boot()` IGNORES the current URL hash (`#world/<id>`) and enters `lastWorld` — so a reload restores whichever world ANY concurrent session entered last. The three observed failure shapes (wrong-world dock miss, worlds-list bounce when the foreign world was deleted, render-ready timeout in a busy foreign world) are one mechanism. This is a real product defect independent of the e2e: reloading a deep-linked world URL discards the URL and teleports to `lastWorld`. The Task-1 watchdog is exonerated (every captured trace shows Welcome arriving in <1s; no watchdog kill ever observed) and keeps its shipped fixed 10s window.

**Files:**
- Modify: `src/client/shell/src/App.svelte` (boot(): route-first world resolution)
- Test: `src/client/shell/src/lib/` — App-level boot logic is currently untested directly; put the covering test where the existing shell test conventions allow (a small extraction is acceptable if App.svelte's inline `boot()` cannot be exercised — prefer the smallest testable seam, e.g. exporting a pure `resolveBootWorld(route, lastWorld, worlds)` helper from a lib module App.svelte consumes)
- Modify: `.claude/skills/shadowcat-codebase-client-shell/SKILL.md` (boot seam: route-first resolution)
- Modify: `docs/CLOSED_BUGS.md` (new entry: reload/deep-link world-route restoration; cite this task's commit + the trace evidence)
- Modify: `docs/POST_WORK_FINDINGS.md` (forensic entry: the Task-3/Task-4 panels failures were THIS pre-existing product defect surfacing under the shared-account parallel suite — NOT a regression from this branch's code; corrects Task-3's report adjudication (which also misquoted the brief's blocking criterion) and records the falsified watchdog hypothesis + binary-staleness confound (a reverted source edit left a 60s watchdog baked in the built binary — e2e:build embeds dist/ into the binary, so source reverts without rebuilds leave stale artifacts))
- Modify: `docs/TODO.md` (render-ready audit backlog: consider per-worker e2e accounts so parallel workers stop sharing one user's `global` slice — the deeper test-hygiene fix; the boot fix already makes reload deterministic)

**Semantics (the single statement):** on boot, if the current route (URL hash) is a world route, enter THAT world id — `lastWorld` is not consulted; `lastWorld` seeds ONLY a bare/non-world load (`/`, entry routes). If the route's world id is absent from `listWorlds()` (deleted/revoked), fall through to the existing stale-`lastWorld` handling (clear + worlds list). `setLastWorld` semantics unchanged (still written on entry, still the cross-load seed for bare loads).

**Steps:** (1) failing test for the resolution rule (route-world wins over lastWorld; bare load uses lastWorld; missing route-world falls back to worlds list); (2) implement in `boot()` — read `currentRoute()` before the `lastWorld` branch; (3) gates: `pnpm --filter @shadowcat/shell test && typecheck && pnpm lint && pnpm -r test`; (4) regression gate: rebuild (`pnpm --filter @shadowcat/shell e2e` includes e2e:build) and run the FULL shell e2e THREE times — all three 16/16; (5) docs per Files; (6) commit `fix(shell): boot honors the URL's world route; lastWorld seeds only bare loads`; (7) review pair.
