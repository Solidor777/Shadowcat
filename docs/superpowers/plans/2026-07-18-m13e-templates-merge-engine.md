...
- **`/base` is hardcoded `OwnerOrGm` egress visibility, non-overridable, on both redaction chokepoints** (`filter_properties` whole-doc, `collect_hidden`/`redact_change` field-level broadcast) — added to Task 2 after buddy-check found `base` was an unredacted leak channel for content hidden elsewhere in the document. Only the doc's owner or a GM ever receives `base`; everyone else gets it nulled (whole-doc) or has the `/base` FieldChange dropped entirely (broadcast), unconditionally, regardless of `property_overrides` (spec §3.3).
