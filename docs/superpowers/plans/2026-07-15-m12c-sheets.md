...
> **Dispatcher amendment (Task 8 implementer finding, 2026-07-15):** two stale patterns below were
> corrected repo-wide (Tasks 8/9/10 share both): (1) `ctx.contributions.contribute(c, { module })`
> is a 2-arg call, but the real `ModuleContext.contributions.contribute` (`modules.ts` `activate()`)
> is a 1-arg closure that auto-injects the module id — every first-party module already calls it
> with 1 arg. Fixed to 1-arg calls in Tasks 8/9/10's `index.ts` snippets, and each task's
> registration test now stubs `contributions` as the real 1-arg wrapper shape (mirrors the fixed
> `sheet-fallback/src/index.test.ts`) rather than the raw `ContributionRegistry.contribute(c,opts)`
> 2-arg signature. (2) `<section role="dialog">` triggers `svelte(a11y_no_noninteractive_element_
> to_interactive_role)` — changed to `<div role="dialog">` in all three sheet components' markup
> (Tasks 8/9/10). Task 11's `defaultModuleOrder.test.ts` addition is unaffected (doesn't assert
> `module`, and `ContributionRegistry.contribute`'s `opts` param already defaults to `{}`).
...
describe("module-sheet-actor registration", () => {
  it("registers ActorSheet under shadowcat.sheet:actor at priority 0", () => {
    ...
    // self-declares it, matching every other first-party module (M12c Task 8 finding).
    sheetActor.register({
...
> **Dispatcher amendment (Task 9 buddy-check Critical, 2026-07-15):** `doc`/`system` derived
> reads below are now pre-patched with a `createSubscriber`/`subscribe()` reactive bridge — the
> Task 9 buddy-check (empirically reproduced, spec reviewer conceded on independent re-trace)
> found that reading `ctx.documents` directly inside a `$derived` freezes the value at the
> component's first read (a plain-callback store, not a Svelte rune) and never observes later
> edits, including the sheet's own prior edits in the same session — corrupting the OCC `old`
> on any second edit, and silently reverting a sibling sub-field on any compound-value edit.
> Mirrors the established `GameSettingsPanel`/`FactionsPanel`/`ConditionsPanel` convention. The
> same fix is being applied as a follow-up to the already-merged Task 8 (`FallbackSheet.svelte`)
> and to Task 9 (`ActorSheet.svelte`, in-flight buddy-check fix).
...
describe("module-sheet-item registration", () => {
  it("registers ItemSheet under shadowcat.sheet:item at priority 0", () => {
    const contributions = new ContributionRegistry();
    // Mirrors the real ModuleContext.contributions wrapper — see Task 9's identical note.
    sheetItem.register({
...
<script lang="ts">
  ...
  // reading it directly freezes at first read and never observes later edits (incl. this
  // sheet's own), corrupting compound-field OCC pre-images on a second edit (M12c Task 9
  // buddy-check Critical). Mirrors GameSettingsPanel/FactionsPanel/ConditionsPanel: one
  // subscribe() call, invoked as the first statement of every $derived reading ctx.documents.
