...
**Dispatcher choice (execution phase):** the user explicitly directed "You are the dispatcher" — the coordinating session (Sonnet 5, this session's default effort) owns the subagent-driven-development dispatch loop mainline, rather than delegating to a generic `sdd-dispatcher` subagent. Per-task implementer/reviewer roles remain the named `shadowcat-*` agents above — this choice only concerns who runs the dispatch loop itself.
