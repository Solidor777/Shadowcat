...
**Buddy-check outcome (Task 2, done 2026-06-18, findings folded in):** Two blind reviewers converged; confidentiality confirmed sound (re-eval re-filters under the connection's own `ctx`; registry task-local; ids self-keyed — no leak). Fixed two Important findings — debounce *starvation* (the deadline was re-armed on every Event → never fired under a sustained stream; now leading-edge only) and inline head-of-line blocking (bounded by the leading-edge fix; offload documented as a TODO) — plus Minors: fingerprint now includes `updated_at`, duplicate `request_id` rejected, query length/term caps. One reviewer's "unbounded `limit`" finding was a false positive (`search` clamps `limit` to 1..=100 before `Vec::with_capacity`). Committed in `fix(m6c-2): address egress-engine buddy-check findings`.
