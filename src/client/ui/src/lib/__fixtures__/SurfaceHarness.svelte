<script lang="ts">
  ...
  setAppContext({ contributions: registry, store: new DocumentStore(), world: "test", role: "gm", t, assets: new AssetResolver(), onAssetChanged: () => () => {}, subscribeScene: () => ({ unsubscribe() {} }), dispatchIntent: () => {}, scene: new SceneInteractionBridge(), leaveWorld: () => {} });
</script>
