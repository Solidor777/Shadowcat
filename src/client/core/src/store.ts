...
export class DocumentStore {
  ...
  applyCommand(cmd: WireCommand): void {
    for (const op of cmd.ops) this.applyOp(op);
    this.appliedSeq = cmd.seq;
    this.emit();
  }

  private applyOp(op: WireOperation): void {
    switch (op.op) {
      case "create":
        this.docs.set(op.doc.id, op.doc);
        break;
      case "delete":
        this.docs.delete(op.doc.id);
        break;
      case "update": {
        const cur = this.docs.get(op.doc_id);
        ...
        this.docs.set(op.doc_id, DocumentSchema.parse(whole));
        break;
      }
    }
  }
