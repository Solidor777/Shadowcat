...
export class DrawingView {
   ...
   * `drawingShapeSpec` resolves to `null` (an unrecognized `shape.kind`, or a `rect`/`ellipse` with
  ...
  reconcile(): void {
    ...
    for (const doc of sceneScopedDocs(this.store, "drawing", this.viewedSceneId, this.viewedLevel)) {
      const spec = drawingShapeSpec(doc);
      if (!spec) continue;
      seen.add(doc.id);
      this.ids.add(doc.id);
 ...
 * Create op with a missing or non-numeric coordinate reaches `drawingShapeSpec` on the authoring
 * client before the server has validated anything. Guarded on the RAW authored points, before
 * tessellation, matching `regionShapeSpec` and `WallView.toSpec` — see the guard's own comment for
 * why the placement, not just the presence, is load-bearing. Exported so `scene-tools`' hit-test
 * can reuse the SAME tessellation this view draws from, rather than forking a second copy of the
 * shape math.
 * @param doc The `drawing` document to convert.
 * @returns A `ShapeNodeSpec` for the `drawings` layer, or `null` if it can't be rendered.
 * @example
 * ```ts
 * import { drawingShapeSpec } from "@shadowcat/render";
 * import type { WireDocument } from "@shadowcat/core";
 *
 * declare const doc: WireDocument;
 * const spec = drawingShapeSpec(doc); // null if doc.engine.shape is absent or malformed
 * ```
 */
export function drawingShapeSpec(doc: WireDocument): ShapeNodeSpec | null {
  const s = doc.engine as DrawingEngine | undefined;
  if (!s?.shape) return null;
  const { kind, points } = s.shape;
  // Checked on the RAW authored points, before tessellation — matching `regionShapeSpec` and
