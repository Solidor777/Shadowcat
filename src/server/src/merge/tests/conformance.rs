...
//! first-seen traversal order, `null`-valued `base`/`engine` envelope
//! keys dropped (absent and null are interchangeable to the merge; the
//! client harness applies the same normalization), and every `owner_standing`
//! key stripped from a `/base` refresh's stored value — the corpus is
//! generated by the client's own merge engine, which never resolves an
//! `OwnerStanding` (a server-only access fact; see
//! `crate::merge::bands::StoredBase`), so the expected side never carries the
//! key and the twin's actual output must match on the snapshot content alone.
...
/// Strip every `owner_standing` key found anywhere in `v` — the field a
/// `/base` refresh's `StoredBase` value carries alongside the plain
/// `MergeBase` keys `snapshot_base` produces (`#[serde(flatten)]`), which the
/// client-generated corpus never expects (see the module doc comment).
fn strip_owner_standing(v: &mut Value) {
    match v {
        Value::Object(obj) => {
            obj.remove("owner_standing");
            for val in obj.values_mut() {
                strip_owner_standing(val);
            }
        }
        Value::Array(arr) => {
            for e in arr {
                strip_owner_standing(e);
            }
        }
        _ => {}
    }
...
fn run_case(case: &Value) -> Value {
    ...
    strip_owner_standing(&mut normalized);
    normalized
}
