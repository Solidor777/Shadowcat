...
describe("ItemSheet dice roll-to-chat", () => {
  it("posts /roll to chat for a dice-notation field value", async () => {
    ...
    const context = setAppContextForTest({ documents, chat: { send: (o) => { sent.push(o); return Promise.resolve(); }, edit: () => Promise.resolve(), delete: () => Promise.resolve(), recalc: () => Promise.resolve(), drawTable: () => Promise.resolve() }, canEdit: () => true });
