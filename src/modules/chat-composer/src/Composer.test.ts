...
function ownedActor(id: string, owner: string, name: string): WireDocument {
  return { ...buildActorDoc("w1", name, { displayName: name, visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, id), owner };
}
...
describe("Composer — @doc link insertion", () => {
  it("opens the doc picker, searches, and inserts a [[doc:id|label]] span at the cursor", async () => {
    const hitDoc = { ...buildActorDoc("w1", "My Doc", { displayName: "My Doc", visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, "doc1") };
  ...
  it("inserts a [[token:id|label]] span for a token-doc_type search hit", async () => {
    const tokenDoc: WireDocument = { ...buildActorDoc("w1", "unused", { displayName: "x", visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, "tok1"), doc_type: "token", name: "Goblin" };
    const searchDocuments: AppContext["searchDocuments"] = vi.fn((_q, _opts, onUpdate) => {
  ...
  it("a name that strips to empty falls back to the id prefix, never an empty label", async () => {
    ...
    const hitDoc = { ...buildActorDoc("w1", "[[||]]", { displayName: "x", visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, "doc1") };
    const searchDocuments: AppContext["searchDocuments"] = vi.fn((_q, _opts, onUpdate) => {
      onUpdate([{ document: hitDoc, score: 1, snippet: "" }]);
      return Promise.resolve({ unsubscribe: () => {} });
    });
  ...
  it("a name containing an unbalanced [ does not desync scan_body's bracket-depth tracking", async () => {
    ...
    const hitDoc = { ...buildActorDoc("w1", "Foo [Bar", { displayName: "x", visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, "doc1") };
    const searchDocuments: AppContext["searchDocuments"] = vi.fn((_q, _opts, onUpdate) => {
      onUpdate([{ document: hitDoc, score: 1, snippet: "" }]);
      return Promise.resolve({ unsubscribe: () => {} });
    });
...
describe("Composer — speak-as-token", () => {
  it("sends a token_instance actor_owner and consumes the pending selection", async () => {
    ...
    const token = { ...buildActorDoc("w1", "unused", { displayName: "x", visual: { kind: "image", asset: "a1" }, size: { w: 1, h: 1 }, shape: "square", faction: null, conditions: [], prototype: false, vision: null, light: null, movement: [] }, "tok1"), doc_type: "token", name: "Goblin" };
    const documents = new DocumentStore();
