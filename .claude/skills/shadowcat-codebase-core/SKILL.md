...
- **NEVER FORK A DECISION ACROSS TWO PATHS — the defect class this codebase produces most.**
  ...
  | Fail-open defaults | `execute_move` vs `publish` vs `pathfind` | a `unwrap_or(100.0)` cell size removed from ONE gate, left in the other two — created by the commit that fixed the row above. **Now removed from all three gates**; SIX NON-gate siblings survive — `navmesh_for` (`scene/mod.rs:1173`), `region_field` (`:1364`), `player_lit_mask` (`:1739`), `visible_cells` (`:1895`), `visible_cells_cached` (`:1939`), `enrich_vision_explored` (`ws/conn.rs:739`) — safe only by call ordering. (`scene_grid_sizes` at `:1056` is the intentional defaulting SOURCE, not a survivor.) See `docs/TODO.md` |
