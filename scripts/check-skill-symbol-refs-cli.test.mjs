...
describe("classifySkillSymbolRun", () => {
  it("exits 0 and prints every bucket on a clean run", () => {
    ...
    expect(banner).toContain("2 unpaired");
...
describe("the CLI process itself", () => {
  it("exits NON-ZERO on a seeded broken citation", () => {
    const repoRoot = mkdtempSync(join(tmpdir(), "symbol-refs-cli-"));
...
describe("the bucket enumeration prose", () => {
  ...
  it("names the run-level exclusions beside the buckets, so nothing leaves uncounted", () => {
    expect(RUN_EXCLUSIONS.map((e) => e.label)).toEqual(["block-blanked", "unpaired"]);
  });
});
